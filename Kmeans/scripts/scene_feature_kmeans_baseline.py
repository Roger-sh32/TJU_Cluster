...
KMEANS_ROOT = SCRIPT_DIR.parent
TABLES_DIR = KMEANS_ROOT / "tables"
PICTURES_DIR = KMEANS_ROOT / "pictures"
DATA_ROOT = KMEANS_ROOT / "聚类"
...
for scene_idx, scene in enumerate(tqdm(nusc.scene[:850], desc="Processing scenes")):
    ...
    while True:
        ...
        if map_available and hasattr(nusc_map, "get_records_in_radius"):
            # 交叉口：附近是否有带 turn_direction 的 lane
            try:
                rec_lane = nusc_map.get_records_in_radius(ego_x, ego_y, 3, ['lane'])
                if 'lane' in rec_lane and len(rec_lane['lane']) > 0:
                    for token in rec_lane['lane']:
                        lane_rec = nusc_map.get('lane', token)
                        if 'turn_direction' in lane_rec and lane_rec['turn_direction'] in ['left', 'right']:
                            intersection_flag = 1
                            break
            except Exception:
                pass
...
plt.title("Scene clustering with fixed speed & TTC")
plt.xlabel("PC1")
plt.ylabel("PC2")
plt.grid(True)
plt.savefig(PICTURES_DIR / "kmeans_pca_clusters.png", dpi=150, bbox_inches="tight")
plt.show()
...
print("===== 各簇特征均值 =====")
print(df_with_cluster.groupby("cluster").mean().round(3))

# 如需导出 CSV
output_path = TABLES_DIR / "scene_features_with_clusters_fixed.csv"
df_with_cluster.to_csv(output_path, index=False)
